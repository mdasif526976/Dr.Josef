...
const Login = () => {
    ...
    const {githubSignIn,setUser,signIn,googleSignIn} = useContext(AuthContext);
   ...
   return (
	...
	<form onSubmit={handlesubmit} className="space-y-6 ng-unhrefuched ng-pristine ng-valid">
		...
		<button className="block w-full p-3 text-center
         rounded-sm text-gray-200 bg-sky-600 hover:text-sky-600 hover:bg-white hover:font-bold">Login</button>
	</form>
	<div className="flex items-center pt-4 space-x-1">
		<div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
		<p className="px-3 text-sm text-gray-200">Login with social accounts</p>
		<div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
	</div>
	<div className="flex justify-center space-x-4">
		<button onClick={GoogleSign} aria-label="Log in with Google"
		...
		<button onClick={githubSignIn} aria-label="Log in with GitHub" 
		className="p-3 rounded-sm">
	...
	<p className="text-2x text-center sm:px-6 text-gray-200">Don't have an account? 

		 <Link  to="/signup" className="hover:underline
         text-gray-100"> Sign up</Link>
	</p>
</div>
