...
const Signup = () => {
    const {githubSignIn,createUser,setUser,updateUser,googleSignIn} = useContext(AuthContext);
   ...
   if (token) {
    ...
    return (
        ...
        <div className="w-full max-w-md p-8 space-y-3 rounded-2xl bg-sky-400 text-gray-100">
 ...
 <form onSubmit={handlesubmit} className="space-y-6 ng-unhrefuched ng-pristine ng-valid">
     <div className="space-y-1 text-sm">
         <label htmlFor="email" className="block text-gray-200">Name</label>
         <input type="text" name="name" id="username" 
         placeholder="Enter Your Name" className="w-full px-4 py-3 rounded-md border-gray-700
          bg-gray-200 text-black focus:border-violet-400" />
     </div>
     <div className="space-y-1 text-sm">
         <label htmlFor="text" className="block text-gray-200">image Url</label>
     ...
     <button className="block w-full p-3 text-center
      rounded-sm text-gray-200 bg-sky-600 hover:text-sky-600 
      hover:bg-white hover:font-bold">Sign Up</button>
 </form>
 <div className="flex items-center pt-4 space-x-1">
     <div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
     <p className="px-3 text-sm text-gray-200">Sign Up with social accounts</p>
     <div className="flex-1 h-px sm:w-16 bg-gray-700"></div>
 </div>
 ...
 <p className="text-xl text-center sm:px-6 text-gray-200">Already have an account? 

      <Link to="/login" className="font-bold hover:underline text-gray-100">Login Now</Link>
 </p>
</div>
